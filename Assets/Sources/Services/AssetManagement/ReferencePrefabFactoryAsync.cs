...
namespace Assets.Sources.Services.AssetManagement
{
    public class ReferencePrefabFactoryAsync<TComponent> : IFactory<AssetReferenceGameObject, UniTask<TComponent>>,
        ...
        IFactory<AssetReferenceGameObject, Vector3, Quaternion, Transform, UniTask<TComponent>>
    {
        ...
        public async UniTask<TComponent> Create(AssetReferenceGameObject assetReference, Vector3 position, Quaternion rotation, Transform parent)
        {
            GameObject prefab = await _assetProvider.Load<GameObject>(assetReference);
            GameObject newObject = _instantiator.InstantiatePrefab(prefab, position, rotation, parent);
